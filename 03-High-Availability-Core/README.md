...
Our RHBK cluster is fully operational using the modern, **intra-site clustering** feature (). This is a simple, elegant, and powerful solution where the Keycloak servers themselves form a single, distributed cache across both sites.
...
In this advanced, optional chapter, we will **replace** our intra-site clustering setup with this external Infinispan architecture.
...
| **Key Config** | cache=ispn,  | cache-stack=tcp, cache-remote-host=... |
...
* **Path A: Native RHBK Multi-Site (Embedded Cache)**  
  ...
  2. **[Configure firewall](/03-Multi-Site-Replication/04-firewall.md)** We will configure the ports required for commnication. 

* **Path B: External Infinispan Cross-Site (Decoupled Cache)**  
  1. **[Deploy External Infinispan:](/03-Multi-Site-Replication/02-rhbk-ispn-ext-deployment.md)** We will deploy two new Infinispan server containers on our sso-mon VM (one for Site A, one for Site B) using docker-compose.  
  2. **[Re-configure RHBK:](/03-Multi-Site-Replication/02-rhbk-ispn-ext-deployment.md#32-lab-re-configuring-rhbk-to-use-an-external-cache)** We will update our keycloak.conf on all four RHBK nodes to disable the native clustered architecture and point to these new external servers instead.
